<?php
 ob_start();
	global $post,$pwyw_bundle_price, $gateway, $wpdb, $besecure, $discount_code, $pmpro_level, $pmpro_levels, $pmpro_msg, $pmpro_msgt, $pmpro_review, $skip_account_fields, $pmpro_paypal_token, $pmpro_show_discount_code;			
	...
	elseif(empty($pmpro_level))
	{
	 ...
	 if((float)$pwyw_bundle_price > 0){
           $pmpro_level->initial_payment = $pwyw_bundle_price;
        }
			...
			{								
				...
				{				
					...
					if($pmpro_requirebilling || ($gateway == "check" && !pmpro_isLevelFree($pmpro_level)))
					{
						...
						$morder->InitialPayment = isset($_SESSION['pwyw_bundle_price'])&&(float)$_SESSION['pwyw_bundle_price']>0?$_SESSION['pwyw_bundle_price']:$pmpro_level->initial_payment;
		...
		{				
			...
			if(pmpro_changeMembershipLevel($custom_level, $user_id))
			{
				...
				//var_dump($current_user);
                  ...
                  //              $_SESSION['pmpro_user'] = $current_user; 
//                                var_dump($_SESSION['pmpro_user']);
//                                die;
//ob_end_clean();
wp_safe_redirect($rurl);
				//var_dump(wp_redirect($rurl));
