...
abstract class PayPal_Digital_Goods {
	...
	protected function get_payment_details_url( $action, $profile_or_transaction_id = '' ){
		...
		if( empty( $this->token ) && ( isset( $_GET['token'] ) || isset( $_POST['token'] ) ) )
			$this->token = !empty($_GET['token']) ? $_GET['token'] : $_POST['token'];
	...
	protected function call_paypal( $action, $profile_id = '', $status = '' ){
		...
		if( $parsed_response['ACK'] == 'Failure' )
			throw new Exception( "Calling PayPal with action $action has Failed: " . $parsed_response['L_LONGMESSAGE0'] );
	...
	public function get_script( $args = array() ){
		...
		$file = plugin_dir_url(__FILE__) . 'dg.js';

		$dg_script  = '<script src ="'.$file.'" type="text/javascript"></script>'
					. '<script>'
	...
	public function get_buy_button( $args = array() ){
		...
		if( $args['type'] == 'anchor' ) {
			...
			$button = '<a href="' . $args['href'] . '" id="' . $args['id'] . '" alt="' . $args['alt'] . '"></a>';
		} else {
			$button = '<input type="button" style="" id="' . $args['id'] . '" alt="' . $args['alt'] . '">';
		}
