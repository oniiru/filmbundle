...
class PWYW_Customer_List_Table extends WP_List_Table {
    ...
    function get_data($current_page, $per_page) {
        ...
        $sql = "SELECT p.`id`, c.`display_name` customer,p.`cid`,cu.`alias`,cu.`is_twitter`, p.`sum` payd, b.`title` bundle,p.`bundle` bid, (p.`average_price`>p.`sum`) above_average FROM {$this->pwyw_payment}  p
                 ...
                 {$query} {$order} LIMIT {$offset},{$per_page}";
